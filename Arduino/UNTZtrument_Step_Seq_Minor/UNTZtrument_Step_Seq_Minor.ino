...
static const uint8_t PROGMEM
  note[8]    = {  72, 70, 69, 67, 65, 63, 62,  60 },
  note2[8]   = {  77, 75, 74, 72, 70, 68, 67,  65 },
          ...
          */ 
          if (grid[x] & mask) { // Already set?  Turn off...
            ...
            if (curnote == 0) {
              ...
              noteOff(pgm_read_byte(&channel[y]), pgm_read_byte(&note2[y]), 127);
            }
          } else { // Turn on
            grid[x] |= mask;
    ...
    for(uint8_t row=0, mask=1; row<8; row++, mask <<= 1) {
      if(grid[col] & mask) {
        if (curnote == 0) {
          ...
          noteOff(pgm_read_byte(&channel[row]), pgm_read_byte(&note2[row]), 127);
        }
      }
    }
    ...
    for(uint8_t row=0, mask=1; row<8; row++, mask <<= 1) {
      if(grid[col] & mask) {
        if (curnote == 0) {
          ...
          noteOn(pgm_read_byte(&channel[row]), pgm_read_byte(&note2[row]), 127);
        }
      }
    }
    prevBeatTime = t;
